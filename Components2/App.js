...
function App() {
    ...
    // NEW HANDLER: Deletes a PCB from the Supervisor's In-Action List (used by Admin)
    const deleteInActionPCB = (serialNumber) => {
        setInActionPCBs(prevPCBs => {
            return prevPCBs.filter(pcb => {
                const pcbKey = pcb._pcb_key_id || PCB_SERIAL_KEY_FALLBACK;
                return pcb[pcbKey] !== serialNumber;
            });
        });
    };
    ...
    const handleAssignWork = (serialNumber) => {
        ...
        const updatedPCBs = inActionPCBs.map(pcb => {
            ...
            if (pcb[pcbKey] === serialNumber) {
                if (pcb.isWorkAssigned) {
                    ...
                    pcbToEdit = { ...pcb, isWorkAssigned: true }; 
                    return pcbToEdit;
                }
            }
            return pcb;
        ...
        if (alreadyAssigned) {
            alert("Work has already been permanently assigned and mapped for this PCB. Opening editor for review.");
            setViewingPCB(pcbToEdit);
        } else if (pcbToEdit) {
    ...
    const renderDashboard = () => { 
        ...
        switch (role) {
            case 'admin':
                // Pass the new deletion handler
                return <AdminDashboard onLogout={handleLogout} role={role} addInActionPCBs={addInActionPCBs} deleteInActionPCB={deleteInActionPCB} />;
            case 'operator':
                return <EmptyDashboard onLogout={handleLogout} role={role} />;
            case 'supervisor':
                    ...
                    <AppBar position="static" color="primary">
                        <Toolbar>
                            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
                                Multi-Role Production System
                            </Typography>
                            <Box>
                                <Button color="inherit" onClick={() => setRole('admin')} variant={role === 'admin' ? 'outlined' : 'text'}>Admin</Button>
                                <Button color="inherit" onClick={() => setRole('supervisor')} variant={role === 'supervisor' ? 'outlined' : 'text'}>Supervisor</Button>
                                <Button color="inherit" onClick={() => setRole('operator')} variant={role === 'operator' ? 'outlined' : 'text'}>Operator</Button>
                            </Box>
                        </Toolbar>
                    </AppBar>
