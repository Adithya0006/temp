...
export default function AddProject() {
  ...
  const pushToInactive = () => {
    const selected = masterList.filter(item => selectedMasterIds.includes(item.id));
    const serials = selected.map(i => i.serialNo);
    ...
    return (
      <Paper variant="outlined">
        <Box sx={{ p: 1, bgcolor: '#f8f9fa', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="caption" sx={{ fontWeight: 'bold' }}>{filterVal} (TOTAL: {categoryData.length})</Typography>
            {isMaster && !isPreview && (
              <Stack direction="row" spacing={1} alignItems="center">
                <Typography variant="caption" sx={{fontWeight:'bold'}}>SELECT PAGE</Typography>
                <Checkbox size="small" checked={allOnPageSelected} onChange={(e) => handleSelectGroup(e.target.checked, paginated)} />
              </Stack>
            )}
        </Box>
              ...
              {paginated.map((row) => (
                <TableRow key={row.id} hover>
                  ...
                  {tab === "inaction" && (
                    <TableCell align="center">
                      <Button size="small" variant="outlined" onClick={() => {
                         axios.put(`${API_BASE}/api/pcb/update-status`, { serialNos: [row.serialNo], status: "New" }).then(() => fetchAllData());
                      }}>Restore</Button>
                    </TableCell>
                  )}
                </TableRow>
    ...
    <Box sx={{ width: '100%', bgcolor: '#f4f6f8', minHeight: '100vh', p: 3 }}>
      <Paper elevation={0} sx={{ border: '1px solid #e0e0e0', borderRadius: 2 }}>
        ...
        {tab === "preview" && (
          <Box sx={{ p: 2 }}>
            <Stack direction="row" justifyContent="space-between" sx={{ mb: 2 }}>
              <Typography variant="subtitle2">Parsed Records: <strong>{fullPreviewData.length}</strong></Typography>
              <Button variant="contained" onClick={saveToMaster} disabled={isUploading}>{isUploading ? "Uploading..." : "Save to DB"}</Button>
            </Stack>
            {isUploading && <LinearProgress variant="determinate" value={uploadProgress} sx={{ mb: 2 }} />}
            {renderTable(fullPreviewData, false, "All", true)}
        ...
        {(tab === "master" || tab === "inaction") && (
          <Box sx={{ p: 3 }}>
            <Stack direction="row" spacing={2} sx={{ mb: 2 }} alignItems="center">
              ...
              <TextField size="small" placeholder="Search..." fullWidth onChange={(e) => setSearchQuery(e.target.value)} InputProps={{ startAdornment: <SearchIcon sx={{ mr: 1, color: 'gray' }} /> }} />
              {tab === "master" && (
                <Button variant="contained" color="warning" disabled={!selectedMasterIds.length} onClick={pushToInactive} sx={{ minWidth: 150 }}>
