...
const COLUMNS = [
   "PCB Process Name", "Actions", "Assigned To", 
  "Assigned To Name", "Initial", "MRL", "MRL Expiry"
];
...
export default function FlowAssigment() {
  ...
  const fetchSuggestions = useCallback(async () => {
    try {
      ...
      // console.log("ops: ",ops)
      setOperatorSuggestions(ops.data || []);
      setProcessSuggestions(procs.data || []);
    } catch (err) {
  ...
  sx={{
    borderRadius: '8px',
    bgcolor: '#059669', // A more professional, slightly deeper Emerald
  ...
  Add New Assignement
            ...
            <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e2e8f0', overflow: "hidden" }}>
              <TableContainer sx={{ maxHeight: '60vh' }}>
                <Table stickyHeader>
                  <TableHead >
                    <TableRow sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>
                      {COLUMNS.map((col) => <TableCell key={col} sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>{col}</TableCell>)}
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {loading ? <TableRow><TableCell colSpan={9} align="center" sx={{ py: 4 }}><CircularProgress /></TableCell></TableRow> : 
                      ...
                      <TableRow hover key={row.id || index} >
                        ...
                        <TableCell sx={{opacity:0.8,fontWeight:600}}><Chip label={row.pcbProcessName} size="small" variant="outlined" color="primary" /></TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>
                        <Button size="small"  sx={{width:"1rem"}} color="primary" onClick={() => handleOpenEdit(row)} ><EditIcon fontSize="small"  /> </Button>
                            <Button size="small" sx={{width:"1rem"}}  color="error" onClick={() => handleDelete(row.id) }><DeleteIcon fontSize="small" /></Button>
                       
                        </TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.assignedTo}</TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.assignedToName}</TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.assignedToNameInitial}</TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.assignedToNameMRL}</TableCell>
                        <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.assignedToNameMRLExpiry}</TableCell>
      ...
      {Bid === "2" && (
        <Card elevation={0} sx={{ borderRadius: 4, mt: 2, maxWidth: 500, mx: 'auto', border: '1px solid #e2e8f0', p: 4, textAlign: 'center' }}>
            ...
            <Button variant="contained" sx={{ mt: 3 }} disabled={!file || isUploading} onClick={saveToMaster}>{isUploading ? "Uploading..." : "Save Flow"}</Button>
        </Card>
      )}

      {/* --- THE MODAL --- */}
      <Dialog open={editDialogOpen} onClose={() => !isProcessing && setEditDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle sx={{ fontWeight: 800 }}>{selectedRow?.id ? "Edit Entry" : "New Assignment"}</DialogTitle>
              ...
              onChange={(e, val) => {
                ...
                if (val) {
                  setSelectedRow({
                    ...
                    assignedToNameInitial: val.operator_initial, 
                    assignedToNameMRL: val.operator_mrl, 
                    assignedToNameMRLExpiry: val.operator_mrl_expiry
                  });
                }
              }}
