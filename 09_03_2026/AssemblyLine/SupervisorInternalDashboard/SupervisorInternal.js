...
import { AccountCircle, ConstructionOutlined, Dashboard } from "@mui/icons-material";
import ArrowBackIosNewIcon from '@mui/icons-material/ArrowBackIosNew';
import ManageMrls from "./ManageMrls";
// import ManageMrls from "./ManageMrls";
...
const SupervisorInternal = ({
  ...
  const [tracdata,setTracData]=useState({ 
    Inaction_PCB_Count: 0, 
    Assigned_PCB_Count: 0, 
    Completed_PCB_Count: 0, 
    MasterList_PCB_Count: 0, 
    ...
    if (currentView === "assignTask") {
      ...
      axios.get(fetchAssignedPCB)
        .then((response) => {
          ...
          // console.log("dfghojfnggndg",response?.data)
...
useEffect(() => {
  ...
  const navCards = [
    ...
    { 
      id: "ManageMRLs", 
      title: "Manage Operators MRL Levels", 
      desc: "Renewal of Expired MRLs",
  ...
  const renderAssignPCB = () => {
    ...
    const handleAssign = async () => {
      // console.log("am inside handleassign 393")
    ...
    return (
      <Box sx={{ maxWidth: "100%", mx: "auto" }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="h6">Available PCBs</Typography>
        </Box>
  ...
  const renderHomeGrid = () => (
    <Container maxWidth="xl" sx={{ height: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
  ...
  const renderModuleContent = () => {
    ...
    switch (currentView) {
      ...
      const chartData = [
        { name: "Yet To Assign", value: taskgraph.Inaction_PCB_Count || 0 },
      ...
      const filteredTraceData = traceData.filter(row => {
          ...
          return (
             ...
             (row.currentProcessOpeName || "").includes(query)
          );
      });
      ...
      const handleCardClick = async (title, value) => {
        ...
        try {
          ...
          const data = response.data;
          // console.log("Data from API:", data); // Check the data structure
        ...
        {/* <Button
          ...
          sx={{
            ...
            color: 'gray',
            cursor: 'pointer',
            m: 6, // Increased margin for better spacing
            zIndex: 1, // 
        ...
        <TableContainer component={Paper}>
              ...
              {data.length > 0 ? (
                data.map((item) => (
                  ...
                  <TableRow key={item.serialNo}>
                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{item.Serial_No}</TableCell>
                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{item.partNumber}</TableCell>
                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{item.productionOrder}</TableCell>
                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{item.Type}</TableCell>
                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{item.description}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell colSpan={5}>No PCB data available.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
          ...
          setDialogContent(`Error fetching details: ${error.message}`);
                    ...
                    <Grid container spacing={4}>
                        ...
                        <Card sx={{ height: "100%", borderRadius: 4, boxShadow: THEME.shadowSoft }}>
                            ...
                            {loading ? (
                              ...
                              <Paper
                                onClick={() => handleCardClick('Inaction', taskgraph.Inaction_PCB_Count || 0)}
                                elevation={0}
                                sx={{ p: 2, borderRadius: 2, bgcolor: '#f1f5f9', borderLeft: `6px solid ${COLORS[0]}`, cursor: 'pointer' }}
                              >
                                <Typography variant="subtitle2" color="text.secondary" fontWeight="600">
                                  Inprogress PCBs
                                </Typography>
                                <Typography variant="h4" fontWeight="800" color="text.primary">
                                  {taskgraph.Inaction_PCB_Count || 0}
                                </Typography>
                              </Paper>
                        
                              <Paper
                                onClick={() => handleCardClick('Assigned', taskgraph.Assigned_PCB_Count || 0)}
                                elevation={0}
                                sx={{ p: 2, borderRadius: 2, bgcolor: '#ecfdf5', borderLeft: `6px solid ${COLORS[1]}`, cursor: 'pointer' }}
                              >
                                <Typography variant="subtitle2" color="text.secondary" fontWeight="600">
                                  Assigned PCBs
                                </Typography>
                                <Typography variant="h4" fontWeight="800" color="text.primary">
                                  {taskgraph.Assigned_PCB_Count || 0}
                                </Typography>
                              </Paper>
                        
                              <Paper
                                onClick={() => handleCardClick('Completed', taskgraph.Completed_PCB_Count || 0)}
                                elevation={0}
                                sx={{ p: 2, borderRadius: 2, bgcolor: '#fffbeb', borderLeft: `6px solid ${COLORS[2]}`, cursor: 'pointer' }}
                              >
                                <Typography variant="subtitle2" color="text.secondary" fontWeight="600">
                                  Completed PCBs
                                </Typography>
                                <Typography variant="h4" fontWeight="800" color="text.primary">
                                  {taskgraph.Completed_PCB_Count || 0}
                                </Typography>
                              </Paper>
                        
                              <Paper
                                onClick={() => handleCardClick('New', taskgraph.MasterList_PCB_Count || 0)}
                                elevation={0}
                                sx={{ p: 2, borderRadius: 2, bgcolor: '#f8fafc', borderLeft: `6px solid #94a3b8`, cursor: 'pointer' }}
                              >
                                <Typography variant="subtitle2" color="text.secondary" fontWeight="600">
                                  Total Master List
                                </Typography>
                                <Typography variant="h4" fontWeight="800" color="text.primary">
                                  {taskgraph.MasterList_PCB_Count || 0}
                                </Typography>
                              </Paper>
                            ...
                            <TableContainer sx={{ maxHeight: '65vh' }}>
                                <Table stickyHeader>
                                    <TableHead>
                                        ...
                                        <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>SERIAL NO</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>PART NUMBER</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>CURRENT STAGE</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>OPERATOR</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>Current Stage ID</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>STEP STATUS</TableCell>
                                    <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 'bold' ,fontSize:"1.15rem"}}>OVERALL STATUS</TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                      
                                        {loading ? (
                                             <TableRow><TableCell colSpan={6} align="center" sx={{opacity:0.8,fontWeight:600}}><CircularProgress /></TableCell></TableRow>
                                        ) : filteredTraceData.length === 0 ? (
                                          
                                            <TableRow>
                                                <TableCell colSpan={6} align="center" sx={{opacity:0.8,fontWeight:600}}>
                                                    No Assigned or Completed tasks found
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                          
                                          filteredTraceData.filter(row => row.Overall_PCB_status !== "COMPLETED").map((row, index) => (
                                                <TableRow key={index} hover sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                                                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.serialNo}</TableCell>
                                                    <TableCell sx={{opacity:0.8,fontWeight:600}}>{row.partNumber}</TableCell>
                                                    
                                                    
                                                    {/* Stage Info */}
                                                    <TableCell sx={{opacity:0.8,fontWeight:600}}>
                                                        <Typography sx={{opacity:0.8,fontWeight:600}}>{row.currentProcessName}</Typography>
...
<Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
  {typeof row.currentProcessOpeName === 'string' ? (() => {
    // 1. Clean the string and split into an array
    const names = row.currentProcessOpeName.split(',').map(s => s.trim()).filter(Boolean);
    
    // 2. Handle the IDs safely (check if string first)
    const ids = typeof row.currentProcessOpeStaffno === 'string' 
      ? row.currentProcessOpeStaffno.split(',').map(s => s.trim()).filter(Boolean) 
      : [];
    ...
    return names.map((name, index) => (
      <Typography 
        ...
        sx={{ 
          ...
          display: 'block' // Ensures each name-id pair is on a new line
        }}
      >
        {name}{ids[index] ? ` - ${ids[index]}` : ''}
      </Typography>
    ));
  })() : (
    <Typography sx={{ 
      ...
      {/* If it's already an array, map it; otherwise, show Unassigned */}
      {Array.isArray(row.currentProcessOpeName) 
        ? row.currentProcessOpeName.join(', ') 
        : "Unassigned"}
    </Typography>
  )}
</Box>
                                                ...
                                                <Typography sx={{opacity:0.8,fontWeight:600}} >Stage: {row.currentStepOrder}</Typography>
                                            </TableCell>
                                                    ...
                                                    <TableCell sx={{opacity:0.8,fontWeight:600}}>
                                                        <Chip 
                                                            ...
                                                            size="large"
                                                            color={row.Overall_PCB_status === "Completed" ? "success" : "primary"}
  ...
  sx={{ 
    color: '#546e7a', // Professional Slate
    backgroundColor: '#f5f5f5', 
    width: '40px', 
    height: '40px',
    borderRadius: '10px', // Slightly squared for a modern look
    transition: '0.3s',
    '&:hover': {
...
>
  <ArrowBackIosNewIcon sx={{ fontSize: '1.2rem' }} />
          ...
          <Typography variant="h6" sx={{
            ...
            {currentView === "home" ? "SUPERVISOR DASHBOARD" : navCards.find(c => c.id === currentView)?.title.toUpperCase()}
          </Typography>
