  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        {[
          { label: "OVERALL TASKS DONE", value: Productivity_History.length, icon: <CheckCircle size={20} color="#4f46e5" />, bg: "#eef2ff" },
          { label: "Present In Assemebly Line", value: Assembly_Line_Traceability.length, icon: <Package size={20} color="#059669" />, bg: "#ecfdf5" },
          { label: "Future PCBs(may enter in assembly line)", value: data.inaction, icon: <Activity size={20} color="#d97706" />, bg: "#fffbeb" },
        ].map((kpi, idx) => (
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 3, height: 350 }}>
            <Box sx={{ p: 1.5, bgcolor: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
              <Stack direction="row" alignItems="center" spacing={1}>
                <History size={16} />
                <Typography variant="subtitle2" fontWeight="bold">My Recent Logs</Typography>
              </Stack>
            </Box>
            <Table size="small">
              <TableHead>
                <TableRow>
                <TableCell align="right" sx={{ fontWeight: 'bold', fontSize: '11px' }}>Serial</TableCell>
              ...
              <TableBody>
                {Productivity_History.slice(0, 10).map((log, i) => (
                  <TableRow key={i}>
                    <TableCell sx={{ fontSize: '12px' }}>{log.pcbserial}</TableCell>
                    <TableCell sx={{ fontSize: '12px' }}>{log.stageName}</TableCell>
                    <TableCell align="right" sx={{ fontSize: '10px', fontWeight:"10px" }}>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 'bold', fontSize: '11px' }}>Assigned Date</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {Assembly_Line_Traceability.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, i) => (
                    <TableRow key={i}>
                      <TableCell sx={{ fontSize: '12px', fontWeight: 'bold' }}>{row.serialNo}</TableCell>
                      <TableCell>
                        <Chip label={row.currentStage} size="small" variant="filled" sx={{ height: 18, fontSize: '12px', bgcolor: '#f1f5f9' }} />
                      </TableCell>
