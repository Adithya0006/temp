...
const PCB_SERIAL_KEY_FALLBACK = 'PCB Serial Number'; // Use fallback for display
...
function SupervisorDashboard({ onLogout, role, onCreateOperator, inActionPCBs, handleAssignWork }) { 
    ...
    // --- NEW Logic: Handles click on the Assign Work button ---
    ...
    // Renders the temporary uploaded table (omitted inner rendering for brevity)
    const renderUploadTable = () => { /* ... */ };

    // Renders the FlowControl (Memory) table (omitted inner rendering for brevity)
    const renderFlowControlTable = () => { /* ... */ };


    // Rendering Function for In-Action PCBs
    const renderInActionPCBsTable = () => {
        ...
        // Find the dynamic key stored in the first PCB object, default to fallback
        const currentPcbKey = inActionPCBs[0]._pcb_key_id || PCB_SERIAL_KEY_FALLBACK;
            ...
            <>
                ...
                {selectedPCB && (
                    <div className="pcb-detail-card">
                        <h4>Details for PCB: {selectedPCB[currentPcbKey]}</h4>
                        <p>Mandatory Operations: {selectedPCB.linkedOperations.length}</p>
                ...
                <table className="data-table flow-table">
                    ...
                    <tbody>
                        {inActionPCBs.map((row, index) => {
                            ...
                            const serialNumber = row[currentPcbKey]; // Use the correct serial number
                            
                            return (
                                <tr key={serialNumber || index}> 
                                    {baseColumns.map(colKey => (
                                        <td key={`${serialNumber}-${colKey}`}>
                                            {/* PCB SERIAL NUMBER CLICKABLE */}
                                    ...
                                    {/* Assignment Status */}
                                    <td style={{ fontWeight: 'bold', color: isAssigned ? '#27ae60' : '#f39c12' }}>
                                        {isAssigned ? 'WORK ASSIGNED' : 'PENDING'}
                                    </td>
                                    {/* Assign Action (Independent Button Check) */}
                                    <td>
                                        <button
                                            onClick={() => handleAssignClick(serialNumber)}
                                            disabled={isAssigned} // Check only this row's status
                                            className={isAssigned ? 'assign-btn-disabled' : 'assign-btn-active'}
                                        >
        ...
        <div className="sup-dashboard-wrapper">
            ...
            <div className="sup-dashboard-container">
                <div className="control-tabs">
                    <button onClick={() => setActiveView('creation')} className={`control-tab-btn ${activeView === 'creation' ? 'active' : ''}`}>Account Creation / Staging</button>
                    <button onClick={() => setActiveView('flowcontrol')} className={`control-tab-btn ${activeView === 'flowcontrol' ? 'active' : ''}`}>FlowControl Memory ({flowControlData.length})</button>
