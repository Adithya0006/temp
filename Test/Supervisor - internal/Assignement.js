...
export default function NewAssignmentEditor({
  ...
  // mappings: { "Operation Name": [ { staffNumber, name } ] }
  ...
  const pcbSerial = pcb ? pcb[pcbKey] : "N/A";

  // prepare options list for selects
  ...
  // Load existing assignments (if any) when component mounts / pcb changes
  useEffect(() => {
    if (pcb?.linkedOperations) {
      ...
      // if pcb has linked operations that differ from default operations, merge them
      if (Array.isArray(pcb.linkedOperations) && pcb.linkedOperations.length) {
        // create operations list from pcb if it appears to be a full ops list
        const names = pcb.linkedOperations.map((o) => o["Operation Name"]);
        // if names differ, use pcb's ops as base (preserve numbering)
        if (names.length && names.length !== operations.length) {
          setOperations(
            pcb.linkedOperations.map((o, idx) => ({
              "S.No": o["S.No"] || String(idx + 1),
      ...
      // reset mappings if no linkedOperations
      setMappings({});
    }
  ...
  const handleAddOperation = () => {
    ...
    if (
      operations.some(
        (op) => op["Operation Name"].toLowerCase() === name.toLowerCase()
      )
    ) {
  ...
  const handleMapOperation = () => {
    ...
    if (selectedOperators.length === 0) return alert("Select operator(s)");
    ...
    setMappings((prev) => {
      ...
      selectedOperators.forEach((op) => {
        const alreadyThere = updated.some(
          (x) => String(x.staffNumber) === String(op.staffNumber)
        );
        if (!alreadyThere) {
          updated.push({
  ...
  // remove entire mapping for an operation
  ...
  // remove a single operator from an operation mapping
  ...
  // Build final linkedOperations array for saving back to parent
  ...
  const handleSaveAll = () => {
    ...
    const empty = finalOps.filter((o) => o.assignedTo.length === 0).length;

    if (
      empty > 0 &&
      !window.confirm(`${empty} operation(s) have no operators. Save anyway?`)
    ) {
      return;
    }
    ...
    <Box sx={{ flexGrow: 1, minHeight: "100vh", bgcolor: "#f5f6fa" }}>
      ...
      <AppBar position="static" sx={{ bgcolor: "#1976d2" }}>
        <Toolbar>
          ...
          <Button variant="outlined" color="inherit" onClick={onCancel} sx={{ mr: 1 }}>
            GO BACK
          </Button>
      ...
      <Container maxWidth="xl" sx={{ pt: 3 }}>
        ...
        <Paper sx={{ p: 3, mb: 4 }}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={4}>
              <FormControl fullWidth size="small">
                ...
                <Select label="Operation" value={selectedOperation} onChange={(e) => setSelectedOperation(e.target.value)}>
                  {operations.map((op) => (
                    <MenuItem key={op["Operation Name"]} value={op["Operation Name"]}>
              ...
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    const selectedObjs = operatorOptions.filter((op) => staffNumbers.includes(op.staffNumber));
            ...
            <Grid item xs={12} md={4} sx={{ display: "flex", alignItems: "center" }}>
              <Button variant="contained" onClick={handleMapOperation} fullWidth>
                ASSIGN
              </Button>
            </Grid>
          </Grid>
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 1 }}>
            <TextField size="small" label="New Operation" fullWidth value={newOperationName} onChange={(e) => setNewOperationName(e.target.value)} />
            <Button variant="contained" color="secondary" startIcon={<AddIcon />} onClick={handleAddOperation}>
              Add
            </Button>
          </Box>
        </Paper>
        ...
        <Paper sx={{ p: 3 }}>
          ...
          <Box sx={{ maxHeight: "68vh", overflowY: "auto", border: "1px solid #ddd", borderRadius: 1 }}>
            <TableContainer>
              <Table size="small">
                <TableHead>
                  <TableRow>
                ...
                <TableBody>
                  {operations.map((op) => {
                    const assigned = mappings[op["Operation Name"]] || [];
                      ...
                      <TableRow key={op["Operation Name"]}>
                        ...
                        <TableCell>
                          {assigned.length === 0 ? (
                            ...
                            assigned.map((a) => (
                              <Box key={a.staffNumber} sx={{ display: "inline-block", bgcolor: "#e1f5fe", px: 1, py: 0.3, borderRadius: "8px", mr: 1, mb: 1 }}>
                                {a.name} ({a.staffNumber})
                                <Button size="small" color="error" onClick={() => handleRemoveSingle(op["Operation Name"], a.staffNumber)}>
                                  x
                                </Button>
                              </Box>
                            ))
                        ...
                        <TableCell>
                          <Button color="error" onClick={() => handleRemoveMapping(op["Operation Name"])}>
                            Remove
                          </Button>
                        </TableCell>
                      </TableRow>
